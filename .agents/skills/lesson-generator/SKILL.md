...
```json
{
  ...
  "questions": [
    {
      "id": "q1",
      "prompt": "What does GROUP BY do in an aggregate query?",
      "difficulty": "easy",
      "answer_key": "It groups rows that share the same values so aggregates run per group."
    },
    {
      "id": "q2",
      "prompt": "Write a query that counts orders per customer using GROUP BY customer_id.",
      "difficulty": "medium",
      "answer_key": "SELECT customer_id, COUNT(*) FROM orders GROUP BY customer_id;"
    },
    {
      "id": "q3",
      "prompt": "How would you filter to only customers with more than 5 orders?",
      "difficulty": "hard",
      "answer_key": "Add HAVING COUNT(*) > 5 after GROUP BY."
    }
  ]
}
```
